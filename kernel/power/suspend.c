...
int pm_standby_transition(standby_state_t state)
{
	...
	/*
	 * When leaving sleep, always fire the resume notification first if
	 * the platform supports it. This is to counteract buggy firmware
	 * (e.g., Lenovo) that expects the resume notification to fire always.
	 */
	if ((standby_current == PM_STANDBY_SLEEP) && state < standby_current &&
	    standby_states[PM_STANDBY_RESUME])
		platform_standby_notify(PM_SN_RESUME);
